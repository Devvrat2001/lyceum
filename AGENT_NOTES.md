...
| Branch | `master` — pushing the cont.45 four (`5d73a39` CI fix [unblocks the cont.44 migration], `2eef006` R41-tail ICU meta/delta, `0f6e0bf` R45-tail community, + docs). Prior push `8b4758c` (run 16) FAILED verify → its `account_deletion` migration is UNAPPLIED; this push re-greens + deploys it. **Watch run 17: deploy must run (not skip) so the migration lands.** P6 tails done; R44 email user-owned. P7 R46–R49 opened. R1 TLS user-owned. |
