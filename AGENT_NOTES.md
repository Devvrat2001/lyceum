...
- **6.1 Activate built-but-dark features** (~1 session) — Resend (`lib/email.ts`) · `ANTHROPIC_API_KEY` in prod (demo fallbacks otherwise) · Stripe account activation + Connect (test charges already work) · Mux webhook + signed playback (keys added; `src/lib/video/mux.ts`).
...
- **6.4 Reach & polish** (ongoing) — mobile screens + nav · `next-intl` (en/es) · true offline lesson reading (precache + attempt-sync; shell done) · admin Branding (`admin/branding/page.tsx`, lone `ComingSoon`).
- **6.5 Rich media & tooling** (ongoing) — LIVE→Zoom/Daily · SPEAK→Whisper scoring · PDF worksheet packs + admin Board report.
- **6.6 Engineering debt & deferred refactors** (opportunistic) — the Tier 4–5 carryover, now a real sub-phase: block reorder across lessons (4.2, `teacher.moveBlock`) · drag-template-from-library v2 (4.1) · `BlockSettingsShape` discriminated union (4.5, ⚠ partly mitigated by `SettingsFor<T>`) · real `chosenIndex`/`subIndex` columns (5.3, trigger = first structured-analytics query) · schedule the remaining crons (streak rollover · weekly emails ⛓6.1 · nightly skill re-route · AI-insight gen ⛓6.1 — embeddings sweep already cron'd).
