...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Block reorder shipped — three-tier nested sortable DnD now complete in the builder.** New `teacher.reorderBlocks({ lessonId, blockIds })` mutation (same shape as reorderUnits/reorderLessons; rejects partial reorders; compacts sparse Block.order back to 1..N). `SortableBlock` component, per-lesson nested DndContext. Block CRUD is now fully closed: add (popover) → see (inline list) → reorder (drag) → delete (×). **Earlier same day**: Block CRUD via deleteBlock + course-query-includes-blocks; click-to-add-block v1 (addBlock mutation + AddBlockPopover); entire Phase 1 polish bucket closed (P1-14 filter popovers, P1-15 header search Combobox, P1-13 topic chip URL filter, P1-28 drag-drop course builder). |
...
- [x] P1-28 Drag-drop builder — `@dnd-kit/{core,sortable,utilities}` installed; `teacher.reorderUnits` + `teacher.reorderLessons` + `teacher.reorderBlocks` all persist on drop. Three levels of nested DndContexts: outer (units), per-unit (lessons), per-lesson (blocks) — keeps cross-list drop math impossible by construction. Drag handle = the `drag` icon at every level, PointerSensor activation distance 6px. **Block CRUD also on top of P1-28:** `teacher.addBlock({ lessonId, type })` + `teacher.deleteBlock({ blockId })` mutations. Each lesson row in the builder gets a "+ block" popover (full 15-type catalog grouped Content / Practice / Interactive / Structure; AI quiz tagged purple), a count badge ("N ▦"), and an inline sortable list of the lesson's blocks with per-block × delete buttons. Block delete intentionally leaves order gaps (sparse) — single UPDATE, no $transaction; the next addBlock just picks max+1 over the gap, and reorderBlocks compacts back to 1..N when the teacher actually rearranges. `BLOCK_GROUPS` + `findBlockMeta` extracted to `src/lib/blocks.ts` so the popover and the list share one catalog. Drag-from-block-library-to-lesson is still v2 polish (requires a single top-level DndContext that crosses block library + units, plus collision-detection tuning).
