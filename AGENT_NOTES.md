...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **MCQ block attempts persist server-side.** New `lesson.attemptBlock({ blockId, chosenIndex })` mutation writes a real `Attempt` row (with `blockId` set, `questionId` null), then runs the same XP / streak / milestone-bonus / badge pipeline as the legacy `lesson.attempt`. Schema migration `20260517054327_attempt_polymorphic` makes `Attempt.questionId` nullable and adds `Attempt.blockId` + reverse `Block.attempts` relation + `[blockId, createdAt]` index. Shared logic extracted to `src/server/services/awardForAttempt.ts`. `BlockReader`'s MCQ body now calls the mutation on "Check answer" — UI shows pending state, server-authoritative correctness, +XP / +STREAK chips, milestone badge surface, and an auth-aware error hint. **Earlier same day**: student-facing block reader (VIDEO / READING / MCQ + 12 placeholders); per-type block fields; per-block inspector; block CRUD + reorder; Phase 1 polish bucket closed (P1-14/15/13/28). |
...
| Last passing | `tsc --noEmit` clean. `scripts/probe-block-attempt.ts` round-trips an Attempt with `blockId` set + `questionId` null through the live DB and cleans up. Reorder probe round-trips both unit + lesson order through the live DB. Phase 3 demo checkout creates Order(PENDING) → flips PAID + Enrollment row → success page. Teacher earnings page reads aggregates from real Order rows; EarningsClient renders Connect onboarding CTA / READY FOR PAYOUTS depending on per-teacher `StripeAccount`. **Not browser-verified this session** — `next dev` was up before the schema change, so it must be restarted before any `attemptBlock` mutation will work at runtime (Prisma client module-cache gotcha). |
| In flight | none |
| Next up | Wire more block-type readers (SLIDES, PDF, AI_QUIZ, POLL — mirror the MCQ pattern); OR Phase 4 (parent role, refund self-service, real Stripe smoke with `npm i stripe`); OR Phase 3 polish (1099 export, invoice email — both need email/PDF primitives). Note: `chosenKey` column is now overloaded — holds lettered keys for Question Attempts, numeric indices stringified for Block Attempts. Long-term we'll split or rename when adding analytics, but it's fine for now. |
