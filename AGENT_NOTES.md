...
| Field | Value |
|---|---|
| Last session | 2026-05-20 — **4 commits, NOT yet pushed.** Flushed a large accumulated changeset (held back across prior sessions at user request) plus two new fixes, then committed on request. (1) **Block templates + typed settings + course publishing** (`358b6f8`) — bundles the prior tier batch (`BLOCK_TEMPLATES` catalog, `BlockLibrary` rail, `SettingsFor<T>` typed settings, sandboxed iframes, AI weak-spot quiz gen) with this session's **course-publish bug fix**: courses are created `DRAFT` and every marketplace surface filters `status: "PUBLISHED"`, but the builder's Publish button had no `onClick` and no publish mutation existed — so teacher-authored courses were permanently invisible to students. Added `teacher.setCourseStatus` (publish/unpublish, ownership-checked, idempotent, audited) + wired the header button; `audit.ts` gained a `course.unpublish` kind. (2) **Role-scoped navigation** (`ad27410`) — `MarketChrome` (the only chrome on public `/` + `/course/*`, so every role + anon sees it) showed all-role nav (My Library / Paths / Teach) to everyone → wrong-role clicks bounced to `/login?error=ForbiddenForRole`. Now role-keyed via a `role` prop fed from the server session; homepage hero is role-centric (TEACHER/ADMIN/PARENT get a dashboard hero, students + anon keep the default discovery hero); fixed two homepage CTAs (`Talk to us`→`/admin`, `Add to my path`→`/student`) that pointed at role-forbidden routes. (3) **Test harness committed** (`0d91873`) — vitest (9 files / 52 cases) + playwright (5 specs / 7 cases), authored prior session, committed now. (4) Notes (`b491827`). All four commits `tsc`+`eslint`-clean; none browser-verified; the test suites were committed but not executed this session. |
...
| Branch | `master` — **4 commits ahead of `origin/master`** (`358b6f8` → `b491827`); last pushed is `e970423`. Working tree clean. **Push pending** — not yet requested. |
...
| Last passing | `tsc --noEmit` + `eslint` clean across every file changed this session (course-publish fix, role-scoped nav, role-centric homepage hero). **Not browser-verified** — no browser run this session. The committed vitest (52 cases) + playwright (7 cases) suites were **not executed this session** — run `npm test` / `npm run test:e2e` to confirm green before relying on them. **Still not browser-verified since 2026-05-16** — the whole stretch since then is tsc/eslint + DB-probe-only. |
| In flight | none |
| Next up | **Push the 4 local commits** to `origin/master` once OK'd. **Browser-verify this session's two fixes:** (a) publish flow — create/edit a course → click Publish → confirm it surfaces on the marketplace homepage + search; (b) role-scoped nav — sign in as each of STUDENT / TEACHER / ADMIN / PARENT and confirm the top bar + homepage hero show only that role's links. **Run the committed test suites** (`npm test`, `npm run test:e2e`) — committed but unexecuted this session. Then resume Phase 4: real-Stripe smoke (Tier 2.2, needs creds), invoice email (Tier 2.5, needs Resend). |
