...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **P1-13 topic chip URL filter wired.** Marketplace chips now actually filter the featured grid via `?topic=`. `MARKETPLACE_TOPICS` lives in `src/lib/marketplace.ts`; `marketplace.featured` translates the slug via `topicWhere()` (subject match for most; title-`contains` OR for "test-prep"). Active chip flips to `wf-chip--accent` and toggles back to `/` on second click; "FILTER: <Label> ×" pill + contextual empty state above the grid when filtered. **Earlier same session**: P1-28 drag-drop course builder via `@dnd-kit/{core,sortable,utilities}`; `teacher.reorderUnits` + `teacher.reorderLessons` (ownership-checked, partial-reorder rejected); nested per-unit SortableContext for lessons; PointerSensor activation distance = 6px so single-click toggle still works; `Card` primitive promoted to `forwardRef`. |
