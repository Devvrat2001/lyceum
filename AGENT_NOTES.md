...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Type-specific block fields (VIDEO / READING / MCQ) on top of the inspector.** `BlockInspector` refactored to a single `draft` state and dispatches on `block.type` via `renderTypeFields`. VIDEO: url + caption. READING: markdown body textarea. MCQ: stem + dynamic 2–6 options list with single-correct radio. SortableBlock row gets a one-line hint derived from settings (host for VIDEO URL, "N words" for READING body, "N opts · 1 ✓" for MCQ). Universal label + notes are above and below the per-type fields respectively. Forward-compat: save spreads existing settings so future fields survive round-trip without an editor. **Earlier same day**: per-block inspector v1 (updateBlock + label/notes); block reorder; block CRUD; click-to-add-block; Phase 1 polish bucket closed (P1-14/15/13/28). |
