...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Student-facing lesson reader renders teacher-authored blocks.** Lesson query (`lesson.bySlug`) now includes blocks; `LessonClient` renders them above the existing legacy question flow. New `BlockReader` dispatches on type: VIDEO embeds YouTube/Vimeo via iframe (16:9; falls back to "Open video ↗" link for unknown hosts), READING renders via a 30-line custom mini-markdown (headings, lists, bold, italic — no dep), MCQ shows stem + clickable options with self-check feedback (Try again). Unknown types render a placeholder card so the page never breaks. 4 sample blocks seeded on `multiplying-fractions` so the reader has demo content. **Earlier same day**: per-type block fields (VIDEO / READING / MCQ); per-block inspector v1 (updateBlock + label/notes); block reorder; block CRUD; click-to-add-block; Phase 1 polish bucket closed (P1-14/15/13/28). |
