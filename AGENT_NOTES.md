...
| Branch | `master` — in sync with `origin/master` @ `cda4c82` (cont.27 sextet incl. `cda4c82` enrollCount recompute). **Prod = `cda4c82`** via the first verify+e2e double-gate run (verify 3.5m / e2e 2.0m / deploy 2.0m, all green first try). **Prod Neon DB seeded 2026-06-11** (user ran `DATABASE_URL=<prod> npm run db:seed` — persona reviews/blocks/honest counters verified live: fractions detail ★ 4.8 (4), origami ★ 4.7 (3)). NB: user's Neon account has TWO projects named `lyceum`; prod = the one with recent compute activity, the other is a dead 20-day-old duplicate (deletable). |
