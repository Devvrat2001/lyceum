...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Entire Phase 1 polish bucket closed.** P1-14 filter popovers shipped: reusable `Popover`/`PopoverOption` primitive in `src/components/ui/`; `MarketplaceFilters` wires Grade · Subject · Price into URL searchParams (`?grade=&subject=&price=`), preserves `topic` on every update, toggles off on second click; section header concatenates all active dimensions. Format/Length/Rating chips kept as visibly-disabled placeholders. **Earlier same session**: P1-15 header search Combobox (debounced typeahead, ARIA combobox, keyboard nav); P1-13 topic chip URL filter (chips actually filter the featured grid). P1-28 drag-drop course builder. |
