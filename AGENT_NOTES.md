...
- **6.1 Activate built-but-dark features** (~1 session) — Resend (`lib/email.ts`) · `ANTHROPIC_API_KEY` in prod (demo fallbacks otherwise) · Stripe account activation + Connect (test charges already work) · ✅ **Mux completion webhook DONE 2026-06-02** (`/api/mux/webhook` — instant `video.asset.ready`; set `MUX_WEBHOOK_SECRET` + add the webhook in the Mux dashboard) — **signed playback** (protect paid videos) still deferred.
