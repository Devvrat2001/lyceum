...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Per-block inspector lights up the right pane.** New `teacher.updateBlock({ blockId, settings })` mutation (settings fully replaced, not merged; permissive `z.record(string, unknown)` schema). Builder selects a block on click (orange-bordered with accent-soft glow); right pane swaps from the course-scoped inspector to a `BlockInspector` form with `label` + `notes` fields. Custom labels render on the block row with the type-meta dimmed alongside. The course-level inspector was extracted to a `CourseInspector` helper so the swap is clean. **Earlier same day**: block reorder (third nested SortableContext, teacher.reorderBlocks); block CRUD (deleteBlock, course query includes blocks); click-to-add-block (addBlock + AddBlockPopover); entire Phase 1 polish bucket closed (P1-14/15/13/28). |
