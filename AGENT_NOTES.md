...
| Last passing | `tsc --noEmit` + `eslint` clean across every file changed this session. Storefront + the real-Stripe purchase both **browser-verified end-to-end**. The committed vitest (52) + playwright (7) suites were NOT run this session — run `npm test` / `npm run test:e2e` to confirm green before relying on them. |
...
| Next up | Phase 5 / Tier 6, or user's pick. To send invoice emails for real: `npm i resend` + a real `RESEND_API_KEY` (it logs-and-skips until then). |
| Blockers | Invoice-email live send → Resend signup + `RESEND_API_KEY`. Real Claude path (tutor, quiz gen, course generator, marketplace AI search) → `ANTHROPIC_API_KEY` (demo fallbacks work without). Teacher payouts → the Stripe test account (`acct_1T10dQ…`) is un-activated (`charges_enabled=false`) and Connect isn't enabled; test charges work regardless, but live payouts need Dashboard activation + Connect setup. Parent self-invite token flow → still needs Resend. |
