...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Big session, ten commits, all pushed.** (1) MCQ block attempts now persist server-side via new `lesson.attemptBlock({ blockId, chosenIndex })` + migration `attempt_polymorphic` (Attempt.questionId nullable, Attempt.blockId added). Shared XP/streak/badge logic extracted to `src/server/services/awardForAttempt.ts`. (2) **EnrollPanel** no longer shows "Buy & start" to already-enrolled students — new `course.myStatus` query + "Continue learning →" CTA. (3) **Marketplace homepage** badges owned course cards with "✓ IN LIBRARY" + "Continue →" via new `course.myEnrolledIds` (single query, Set lookup). (4) **Path cards** got 3-state button (Enroll / "N / M OWNED" partial hint / "✓ In your library"). (5) **Three new presentational readers**: SLIDES (Google Slides /edit→/embed normalize), PDF (iframe + open-in-new-tab fallback), SECTION (serif divider). (6) **POLL block** end-to-end via new `BlockVote` model (one vote per (block,user), upsertable). (7) **DISCUSSION block** end-to-end via new `BlockComment` model (flat thread v1, no replies). (8) Sample blocks of all 5 new types seeded on multiplying-fractions via `scripts/seed-sample-blocks.ts` (idempotent). (9) `CLAUDE.md` went from 1 line → 38 lines (stack, commands, gotchas, key paths). (10) Pushed 23 commits to `origin/master`. |
| Phase | **Phase 1: 100% complete.** **Phase 2: complete.** **Phase 3: v1 shipped** (demo + Stripe Connect wired; real mode needs `npm i stripe` + keys). **Block reader coverage 8 / 15** (VIDEO, READING, MCQ, SLIDES, PDF, SECTION, POLL, DISCUSSION). Remaining: QUIZ, SPEAK, AI_QUIZ, SIMULATION, BRANCHING, DRAG_MATCH, LIVE. |
| Branch | `master` — in sync with `origin/master` at `da9982a`. Working tree clean. |
| Dev server | `npm run dev` — port 3000, Turbopack. Postgres port **5433** via Docker. **IMPORTANT:** Prisma client is module-cached at first import — after any `schema.prisma` change, run `npx prisma generate` AND restart `next dev`. This session added **3 migrations** (`attempt_polymorphic`, `add_block_votes`, `add_block_comments`); if a dev server was running before any of those, it MUST be restarted before MCQ submits / POLL votes / DISCUSSION posts work from the browser. |
| Last passing | `tsc --noEmit` clean across all 10 commits. Probes (all written + cleaned-up): `scripts/probe-block-attempt.ts` round-trips polymorphic Attempt; one-off probes verified BlockVote upsert (vote → change-vote → tally via groupBy) and the myEnrolledIds query shape. `scripts/seed-sample-blocks.ts` idempotently seeds all 5 new sample blocks. **Not browser-verified** — every commit landed via tsc + DB probe only; dev server was running pre-migrations and we deliberately did not restart it without you. |
| In flight | none |
| Next up | Highest leverage: **AI_QUIZ block** (wire to `@anthropic-ai/sdk` already in deps; generate questions on the fly, persist via existing `attemptBlock`; needs prompt design + API error handling). Lighter alternatives: **DRAG_MATCH** (interactive via `@dnd-kit`, single "I solved it" attempt — pure interaction, no AI), **LIVE** (scheduled session card with "starts in N" affordance), **QUIZ** (multi-question container, wrapper around N MCQs). Or pivot to **Phase 4** (parent role, refund self-service UI, real-Stripe smoke). |
| Blockers | Real-Stripe mode needs `npm i stripe`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` in `.env.local`. Demo mode runs without any of that. AI_QUIZ needs `ANTHROPIC_API_KEY` (already used by tutor streaming). |
...
- **`Attempt.chosenKey` is dual-purpose.** Legacy Question-based attempts use lettered keys (`"A"`, `"B"`, …). Block MCQ attempts use **stringified positional indices** (`"0"`, `"1"`, …) — POLL's BlockVote does the same. When you eventually need cross-attempt analytics, decide on a normalization (split into `chosenKey String?` + `chosenIndex Int?`, or write a view that resolves both).
