...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **P1-15 header search Combobox shipped.** Debounced typeahead in the marketplace chrome: ≥2-char min, 220ms quiet, hits the existing `marketplace.search` (ILIKE on title/desc/tagline), ↑/↓/Enter/Esc keyboard nav, click-outside closes, top-6 results + overflow note. ARIA combobox+listbox+activedescendant. **Earlier same session**: P1-13 topic chip URL filter (chips actually filter the featured grid via `?topic=`, active class via `wf-chip--accent`, "FILTER ×" pill); P1-28 drag-drop course builder via dnd-kit (reorder units + reorder lessons within unit, optimistic UI, rollback on error, nested per-unit SortableContext, `Card` → `forwardRef`). |
