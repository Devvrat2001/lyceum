...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Massive session: 18 commits total, all pushed.** Two distinct arcs: (A) **Enrollment-state bug class fully closed** across all 3 marketplace surfaces (EnrollPanel, featured cards, path cards). (B) **Block reader coverage went 3 → 15 of 15** — every BlockType in the schema now has a working reader + inspector. Detailed: MCQ persistence (`attemptBlock` + `attempt_polymorphic` migration); SLIDES / PDF / SECTION (presentational); POLL (new `BlockVote` model, upsertable vote); DISCUSSION (new `BlockComment` model, flat thread); AI_QUIZ (Anthropic-generated, cached in settings); DRAG_MATCH (@dnd-kit pool-to-slot, stable shuffle); LIVE (time-aware UPCOMING / LIVE / ENDED with 30s setInterval); QUIZ (curated MCQ deck sharing QuizQuestionCard with AI_QUIZ); SIMULATION (generic embed + link fallback); SPEAK (WebSpeech TTS+STT with text-input fallback for Firefox); BRANCHING (graph editor + state-machine reader). Extracted `zodToJsonSchema` from generator.ts to `src/lib/ai/zodToJsonSchema.ts`. Sample blocks of every new type seeded on multiplying-fractions via `scripts/seed-sample-blocks.ts` (idempotent, marker-label-based). CLAUDE.md went from 1 line → 38 lines. |
| Phase | **Phase 1: 100% complete.** **Phase 2: complete.** **Phase 3: v1 shipped.** **Block reader coverage 15 / 15.** Every BlockType has a working reader + inspector. |
| Branch | `master` — in sync with `origin/master` at `648484c`. Working tree clean. |
...
| Last passing | `tsc --noEmit` clean across all 18 commits. Probes (all written + cleaned-up): `scripts/probe-block-attempt.ts` (polymorphic Attempt), one-off probes for BlockVote upsert + myEnrolledIds. `scripts/seed-sample-blocks.ts` idempotently seeds 12 sample blocks of distinct types on multiplying-fractions (re-run after schema changes; uses MARKER_LABEL on settings for dedup). **Not browser-verified** this whole stretch — every commit landed via tsc + DB probe only. WebSpeech (SPEAK) especially needs cross-browser eye-test since the API surface varies. |
| In flight | none |
