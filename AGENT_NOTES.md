...
| Field | Value |
|---|---|
| Last session | 2026-05-17 — **Block-add v1 (click-to-add) shipped on top of P1-28.** New `teacher.addBlock({ lessonId, type })` mutation (ownership chain through unit→course, order = max+1, settings: {}). `AddBlockPopover` component on each lesson row with the full 15-type catalog grouped Content / Practice / Interactive / Structure (AI quiz gets the purple accent). Block count badge ("N ▦") on each lesson, optimistically incremented after a successful add. The per-unit "+ DROP A BLOCK HERE" hatch retitled to point users at the lesson-level button. Drag-from-block-library-onto-lesson is still v2 polish (needs one top-level DndContext crossing block library + units, plus a collision-detection tweak). **Earlier same day**: Entire Phase 1 polish bucket closed — P1-14 filter popovers, P1-15 header search Combobox, P1-13 topic chip URL filter, P1-28 drag-drop course builder. |
...
- [x] P1-28 Drag-drop builder — `@dnd-kit/{core,sortable,utilities}` installed; `teacher.reorderUnits` + `teacher.reorderLessons` persist on drop; nested sortable contexts (units list outer, per-unit lessons inner) so a lesson drag never reorders units. Drag handle = the `drag` icon, PointerSensor activation distance 6px. **Block-add v1 also shipped (click-to-add):** `teacher.addBlock({ lessonId, type })` mutation; each lesson row gets a "+ block" popover with the full 15-type catalog grouped by Content / Practice / Interactive / Structure (AI quiz tagged purple); block count badge on the lesson row updates optimistically. Drag-from-block-library-to-lesson is still v2 polish (requires a single top-level DndContext that crosses block library + units, plus collision-detection tuning).
