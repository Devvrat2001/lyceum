...
| Field | Value |
|---|---|
| Last session | 2026-05-16 — **P2-07 done: real per-teacher + per-admin AI insights** replace the hardcoded "23% stuck on Lesson 14" decoration. 24h DB cache + Refresh button. Demo fallback still grounded in real stats. |
| Phase | **Phase 1: ~99% done.** Everything from the original AGENT_NOTES checklist landed except drag-drop course builder (P1-28), filter popovers (P1-14), header search combobox (P1-15) — all explicitly low-priority. Phase 2 (AI streaming tutor) is the natural next step. |
| Branch | `main` |
| Dev server | `npm run dev` — port 3000, Turbopack. Postgres port **5433** via Docker. |
| Last passing | `tsc --noEmit` clean; full route matrix; enroll mutation persists `Enrollment` row; correct quiz attempt bumps streak (14 → 15 verified in DB); notification panel renders 3 unread items + empty state; 404 page renders for unknown course slugs. |
| In flight | none |
| Next up | **Phase 2: AI streaming tutor (Anthropic Claude)** OR remaining P1 polish (P1-13/14/15/18/28) |
| Blockers | Phase 2 needs `ANTHROPIC_API_KEY` in `.env.local` |
